...
GA4.prototype.identify = function (identify) {
  ...
  var validReportingIdentity =
    opts.googleReportingIdentity === "userIdSignalsAndDevice" ||
    opts.googleReportingIdentity === "userIdAndDevice";
...
GA4.prototype.track = function (track) {
  ...
  const mapping = customEventMappings.find(
    (mapping) => mapping.segmentEvent === track.event()
  ) || {
    googleEvent: track.event(),
  };
  const parameters = Array.isArray(mapping.parameters)
    ? mapping.parameters.reduce(
        (accum, { googleParameter, segmentParameter }) => ({
          ...accum,
